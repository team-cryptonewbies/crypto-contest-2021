...
def read_data_files(plaintext_path='./data/pt.dat',
                    ciphertext_path='./data/ct.dat'):
    ...
    for _ in range(0x500):
        ...
        ciphertexts = []
        for _ in range(16):
            plaintexts.append(BitArray(uint=plaintext_file.read('uintle:64'), length=64))
            ciphertexts.append(BitArray(uint=ciphertext_file.read('uintle:64'), length=64))
